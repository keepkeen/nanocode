...
from .utils import compact_text, extract_title, html_to_text, keyword_spans
...
class SecureWebFetchTool(AgentTool):
    ...
    def __init__(self, *, policy: SecurityPolicy, audit: AuditLogger | None = None, user_agent: str = "optimized-agent-tools/0.1") -> None:
        self.policy = policy
        self.audit = audit
    ...
    def invoke(self, ctx: ToolContext, **kwargs: Any) -> ToolResult:
        ...
        text = html_to_text(decoded) if "html" in content_type or "xml" in content_type or "<html" in decoded.lower() else decoded
        reduced_text = keyword_spans(text, query) if query else compact_text(text, 6000)
        content = FetchContent(
            ...
            metadata={"raw_length": len(raw), "hostname": urlparse(final_normalized).hostname or ""},
        )
